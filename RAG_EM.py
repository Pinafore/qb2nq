import os
os.environ['TRANSFORMERS_CACHE'] = '/fs/clip-scratch/tasnim/new_fol/temp_cache'
os.environ['HF_DATASETS_CACHE'] = '/fs/clip-scratch/tasnim/new_fol/temp_cache'
import numpy as np
import pandas as pd
import json
import nltk
...
'''
RAGretriever = RagRetriever.from_pretrained("facebook/rag-token-nq", index_name="exact", use_dummy_dataset=True)
...
nq_train = '/fs/clip-scratch/tasnim/new_fol_temp/data.json'
nq_dev = '/fs/clip-scratch/tasnim/new_fol_temp/data.json'
nq_test = '/fs/clip-scratch/tasnim/new_fol_temp/data.json'
...
q_max, a_max = 0, 0
df = pd.read_json(nq_train, orient='records')
...
def my_collate_fn(batch):
  ...
  for i in range(1,batch_size):
    if input_ids.size(dim=1)>batch[i]['input_ids'].size(dim=1):
      target_len = input_ids.size(dim=1)
      batch[i]['input_ids'] = F.pad(batch[i]['input_ids'], (target_len - batch[i]['input_ids'].size(1), 0))
    else:
      target_len = batch[i]['input_ids'].size(dim=1)
      input_ids = F.pad(input_ids, (target_len - input_ids.size(1), 0))
    if attention_masks.size(dim=1)>batch[i]['attention_mask'].size(dim=1):
      target_len = attention_masks.size(dim=1)
      batch[i]['attention_mask'] = F.pad(batch[i]['attention_mask'], (target_len - batch[i]['attention_mask'].size(1), 0))
    else:
      target_len = batch[i]['attention_mask'].size(dim=1)
      attention_masks = F.pad(attention_masks, (target_len - attention_masks.size(1), 0))
...
train = pd.read_json(nq_train, orient='records')
dev = pd.read_json(nq_dev, orient='records')
test = pd.read_json(nq_test, orient='records')
...
QA_data = QADataModule(RAGtokenizer, data_file=data_file_dict, batch_size = 2, q_transform=None, a_transform=remove_punctuation)
...
checkpoint_PATH = '/fs/clip-scratch/tasnim/new_fol_temp/checkpoint2/'
